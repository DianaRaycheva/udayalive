...
jQuery( function ( $ ) {
    ...
    if(typeof LOG_BTN != 'undefined'){
        $('#ws_api_key').next('span').after(LOG_BTN);
        $('#ws_info_wrap').after( LOG_BTN );
    }
    // Subscribe options
    $('#ws_dopt_enabled').change(function(){
        $( this ).closest( 'tr' ).next( 'tr').hide('fast');
    ...
    $('#ws_opt_field').change(function(){
        $( this ).closest( 'table' ).nextAll( 'table').hide('fast');

        if ( $(this).prop('checked') == true ) {
            $( this ).closest( 'table' ).nextAll( 'table').show('fast');
        }
        else {
            $( this ).closest( 'table' ).nextAll( 'table').hide('fast');
        }
    ...
    */
    ...
    $('#ws_sms_credits_notify').click(function(){
        $(this).closest('form').find('table:eq(6)').toggle(500,function(){ validProp(); });
        $(this).closest('form').find('h3:eq(3)').toggle();
    });
    $('input[name="ws_sms_credits_notify"]').each(function(){
        if(!$(this).is(':checked')){
            $(this).closest('form').find('table:eq(6)').hide();
