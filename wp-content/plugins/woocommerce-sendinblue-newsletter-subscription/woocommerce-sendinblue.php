    ...
    class WC_Sendinblue_Integration
    {
        ...
        public function ws_order_status_changed($id, $status = 'new', $new_status = 'pending')
        {
            ...
            if ($this->ws_subscribe_enable == 'yes' && $new_status == $this->customizations['ws_order_event']) {
                ...
                $exist_user = $wc_sib->check_subscriber($order->billing_email); // 'success' or 'fail'

                if( $exist_user != 'success' ) { // we should add new customers only
         ...
         * - All WC emails are sent vis SendinBlue SMTP
         */
        public static function phpmailer_sendinblue_smtp($phpmailer){

            $general_settings = get_option('ws_main_option', array());
            WC_Sendinblue_SMTP::update_smtp_details($general_settings['access_key']);
            if(WC_Sendinblue_SMTP::$smtp_details['relay'] == false)
                return;
            if(self::$ws_smtp_enabled == 'yes'){
                $phpmailer->isSMTP();
                $phpmailer->SMTPSecure = 'tls';
                $phpmailer->Host = WC_Sendinblue_SMTP::$smtp_details['relay'];//'smtp-relay.sendinblue.com';
                $phpmailer->Port = WC_Sendinblue_SMTP::$smtp_details['port'];//'587';
                $phpmailer->SMTPAuth = TRUE;
                $phpmailer->Username = WC_Sendinblue_SMTP::$smtp_details['username'];//$general_settings['account_email'];
                $phpmailer->Password = WC_Sendinblue_SMTP::$smtp_details['password'];//$general_settings['access_key'];
            }
        }
        // custom schedule
