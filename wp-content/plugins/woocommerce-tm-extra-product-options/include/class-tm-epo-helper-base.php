...
final class TM_EPO_HELPER_base {
	...
	public function convert_to_right_icon($label='',$icon='fa-angle-right'){
		$label = str_replace("/", "", $label);
		$label .= '<i class="tm-icon tmfa fa '.$icon.'"></i>';
		return $label;
	}
		...
		if($this->init_filesystem()){
			...
			foreach ($directories as $key => $value) {
				...
				$html .='<div class="tm-mn-wrap-dir tm-row nopadding nomargin">';
					...
					.'<a title="'.__( 'Delete', TM_EPO_TRANSLATION ).'" href="#" data-tm-dir="'.esc_attr($todir).'" data-tm-deldir="'.esc_attr($data_tm_dir).'" class="tm-mn-deldir"><i class="tm-icon tmfa fa fa-times"></i></a>'
					.'</div>';
				$html .='</div>';
			}
			foreach ($files as $key => $value) {
				...
				$html .='<div class="tm-mn-wrap-file tm-row nopadding nomargin">';
					...
					$html .='<div class="tm-mn-op tm-cell col-3">'
					.'<a title="'.__( 'Delete', TM_EPO_TRANSLATION ).'" href="#" data-tm-dir="'.esc_attr($todir).'" data-tm-deldir="'.esc_attr($data_tm_dir).'" data-tm-delfile="'.esc_attr($value["name"]).'" class="tm-mn-delfile"><i class="tm-icon tmfa fa fa-times"></i></a>'
					.'</div>';
				$html .='</div>';
			}
