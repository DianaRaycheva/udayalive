...
final class TM_EPO_BUILDER_base {
	...
	private function init_elements(){
		...
		$this->elements_array=array(
			...
			"textarea"=>$this->add_element(
				"textarea",
				array("required","price","text_before_price","text_after_price","price_type","hide_amount","quantity","placeholder","min_chars","max_chars","validation1")
				),			
			
			"textfield"=>$this->add_element(
				"textfield",
				array("required","price","text_before_price","text_after_price","price_type2","hide_amount","quantity","placeholder","min_chars","max_chars","min","max","validation1")
				),
			...
			"radiobuttons"=>$this->add_element(
				"radiobuttons",
				array("required","text_before_price","text_after_price","hide_amount","quantity","use_url","use_images","changes_product_image","swatchmode","items_per_row","clear_options","options")
				),			

			"checkboxes"=>$this->add_element(
				"checkboxes",
				array("required","text_before_price","text_after_price","hide_amount","quantity","limit_choices","exactlimit_choices","minimumlimit_choices","use_images","changes_product_image","swatchmode","items_per_row","options")
				),			
			...
			"range"=>$this->add_element(
				"range",
				array("required","price","text_before_price","text_after_price","price_type7","hide_amount","quantity","min","max",
					array(
						"id" 		=> "range_step",
						"wpmldisable"=>1,
						"default"	=> "1",
	...
	public final function add_setting_minimumlimit_choices($name=""){
		return array(
					...
					"default"	=> "",
					"type"		=> "text",
					"tags"		=> array( "class"=>"n", "id"=>"builder_".$name."_minimumlimit_choices", "name"=>"tm_meta[tmfbuilder][".$name."_minimumlimit_choices][]" ),
	...
	private function _section_template($out="",$size="",$section_size="", $sections_slides="", $elements="", $wpml_is_original_product=true){
		...
		if($sections_slides!=="" && is_array($elements)){
			...
			$s=0;
			$tabs="";
			$add='<div class="tm-box tm-add-box"><h4 class="tm-add-tab"><span class="fa fa-plus"></span></h4></div>';
			...
			$h = $t0.$t1."<div class='bitem_wrapper'>".$elements."</div>".$t2;
		}
		...
		*/
		if ( !empty( $post_id ) && is_array( $builder ) && count( $builder )>0 && isset($builder['element_type']) && is_array($builder['element_type']) && count($builder['element_type'])>0 ) {
			...
			for ( $_s = 0; $_s < count( $_sections ); $_s++ ) {
											...
											if (!is_null($_titles_base) && !is_null($_values_base) && !is_null($_prices_base) ){
												...
												foreach ($_titles_base as $option_key=>$option_value){
													...
													if($use_original_builder){
														...
														foreach ($c_prices as $ckey => $cvalue) {
															$mt_prefix = TM_EPO_HELPER()->get_currency_price_prefix($ckey);
															$cbvalues["price".$mt_prefix]=TM_EPO_HELPER()->build_array($cvalue[$_option_key],$c_prices_base[$ckey][$_option_key]);
														}
														$t[$_vid][]=$cbvalues;
													}
												}
				...
				$out .= $this->_section_template( $section_html, $this->sizer[$_sections_size[$_s]], $_sections_size[$_s], $_sections_slides[$_s],$elements_html_array,$wpml_is_original_product );
	...
	private function builder_sub_options( $options=array(), $name="multiple_selectbox_options", $counter=NULL , $default_value=NULL) {
		...
		$out  = "<div class='tm-row nopadding multiple_options clearfix'>"
			...
			//. "<div class='tm-cell col-1 tm_cell_url'>".__( "URL", TM_EPO_TRANSLATION )."</div>"
		...
		foreach ( $options["title"] as $ar=>$el ) {
			$out  	.= "<div class='options_wrap'>"
					. "<div class='tm-row nopadding clearfix'>";
			...
			$out .= "<div class='tm-cell col-12 tm_cell_url'><span class='tm-inline-label'>".__( "URL", TM_EPO_TRANSLATION )."</span>".TM_EPO_HTML()->tm_make_field( $o["url"], 0 )."</div>";
